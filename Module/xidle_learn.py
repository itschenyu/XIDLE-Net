...
class NetLearn:
        ...
        with torch.no_grad():
            ...
            for data in dataLoader:
                ...
                for i in range(Y_saliency_pred_norm.shape[0]):
                    ...
                    original_image = X[i].squeeze(0)  # Remove the extra dimension (from [1, 3, 224, 224] to [3, 224, 224])
                    # Convert to HxWxC format for visualization
                    ...
                    saliency_uint8 = (predicted_saliency * 255).astype(np.uint8)

                    Image.fromarray(original_image).save(os.path.join(self.path, f'{counter}_original.png'))

                    Image.fromarray(saliency_uint8).save(os.path.join(self.path, f'{counter}_saliency.png'))

                    saliency_colormap = cv2.applyColorMap(saliency_uint8, cv2.COLORMAP_JET)  # BGR
                    saliency_colormap = cv2.cvtColor(saliency_colormap, cv2.COLOR_BGR2RGB)   # RGB

                    overlay = cv2.addWeighted(original_image, 0.5, saliency_colormap, 0.5, 0)

                    Image.fromarray(overlay).save(os.path.join(self.path, f'{counter}_overlay.png'))
    ...
    def train_judgetment(self):
        ...
        if self.epoch==1:
            ...
            self.accuracy_max = self.metrics_list[-1]
        else:
            ...
            if self.metrics_list[-1]>self.accuracy_max:
                self.accuracy_max = self.metrics_list[-1]
                self.save_net(self.save_path)
                self.epoch_best = self.epoch
    ...
    def test(self):
        ...
        eval_test = self.test_iterate(self.dataAll('Test'))
        eval_test['metrics_class'].compute_classification_report()
        eval_test['metrics_class'].save_classification_report('classification_report', self.save_path)
        eval_test['metrics_class'].save_outputs('classification_results', self.save_path)
