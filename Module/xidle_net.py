...
def segmentation_head(in_channels, out_channels, kernel_size=3, upsampling=1):
    conv2d = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=kernel_size // 2)
    upsampling = nn.UpsamplingBilinear2d(scale_factor=upsampling) if upsampling > 1 else nn.Identity()
    return nn.Sequential(upsampling, conv2d)
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels=0, use_batchnorm=True, do_upsample=True):
        super().__init__()
        self.default_do_upsample = do_upsample
    ...
    def forward(self, x, skip=None, do_up=None):
        if do_up is None:
            do_up = self.default_do_upsample
        if do_up:
            x = self.up(x)
        if skip is not None:
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.decoder3 = DecoderBlock(1024, 1024, skip_channels=1024)  # x+skip3: x(1024, 7, 7) --cat- skip2(1024, 7, 7) -> (1024, 7, 7)
        self.decoder2 = DecoderBlock(1024, 512, skip_channels=512)   # x+skip2: x(1024, 7, 7) --up-> (1024, 14, 14) --cat- skip1(512, 14, 14) -> (512, 14, 14)
        self.decoder1 = DecoderBlock(512, 256, skip_channels=256)   # x+skip1: x(512, 14, 14) --up-> (512, 28, 28) --cat- skip0(256, 28, 28) -> (256, 28, 28)
        self.decoder0 = DecoderBlock(256, 128, skip_channels=128)     # x+skip0: x(256, 28, 28) --up-> (256, 56, 56) --cat- skip0(128, 56, 56) -> (128, 56, 56)
        self.decoder_up = DecoderBlock(128, 64, skip_channels=0)    # x (128, 56, 56) --up-> (128, 112, 112) -> (64, 112, 112)
    ...
    def forward_up_features(self, x, x_downsample):
        ...
        skip0 = x_downsample[0].permute(0, 2, 1).contiguous()
        skip0 = skip0.view(B, -1, self.patches_resolution[0], self.patches_resolution[1])

        skip1 = x_downsample[1].permute(0, 2, 1).contiguous()
        skip1 = skip1.view(B, -1, self.patches_resolution[0] // 2, self.patches_resolution[1] // 2)

        skip2 = x_downsample[2].permute(0, 2, 1).contiguous()
        skip2 = skip2.view(B, -1, self.patches_resolution[0] // 4, self.patches_resolution[1] // 4)

        skip3 = x_downsample[3].permute(0, 2, 1).contiguous()
        skip3 = skip3.view(B, -1, self.patches_resolution[0] // 8, self.patches_resolution[1] // 8)

        x = self.decoder3(x, skip3, do_up=False)
        x = self.decoder2(x, skip2)        
        x = self.decoder1(x, skip1)    
        x = self.decoder0(x, skip0)
        x = self.decoder_up(x, None)

        return x_cls, x

    def forward(self, x):
        x, x_downsample = self.forward_features(x)
        x_cls, x = self.forward_up_features(x, x_downsample)
...
class XIDLE_Chunk(nn.Module):
    ...
    def forward(self, x):
        x = x.squeeze(1)
        logits, x = self.swin(x)

        return logits, x
    
    
class XIDLE_Net(XIDLE_Chunk):
    
    def __init__(self, in_channels, filter_list, out_dict, config, img_size=224, zero_head=False, vis=False):
        self.in_channels = in_channels
        self.filter_list = filter_list
        super().__init__(config, img_size, zero_head, vis)
            ...
            if 'class' in self.out_dict:
                if self.out_dict['class']>0:
                    self.out_classification = classification_head(
                        in_features=1024,
                        mid_features=None, out_features=self.out_dict['class'],
                        dropout_rate=0.25)

            if 'image' in self.out_dict:
                if self.out_dict['image']>0:
                    self.segmentation_head = segmentation_head(in_channels=64, out_channels=self.out_dict['image'], kernel_size=1, upsampling=2.0) # 96 for tiny
    ...
    def forward(self, x):
        ...
        if self.out_dict is None:
            ...
            if 'class' in self.out_dict:
                if self.out_dict['class']>0:
                    average_pool_e5 = self.avgpool(e5.transpose(1,2))
                    average_pool_e5 = torch.flatten(average_pool_e5, 1)
            ...
            if 'image' in self.out_dict:
                if self.out_dict['image']>0:
                    y_image = self.segmentation_head(d2)
                    r.append(y_image)
                else:
                    r.append(self.dummy_tensor)
            else:
...
class XIDLE_Net_preset(XIDLE_Net):
    
    def __init__(self, device, out_dict, loss_dict, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        self.device = device
